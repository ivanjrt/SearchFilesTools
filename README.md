...
**Additional capability:** Automatically discovers and converts Windows Event Log files (`.evtx`) into searchable `.xml` using the Windows `wevtutil` command, so event log contents become searchable too.

**Key features:**

- **Drag-and-drop input** -- drop a folder or a `.zip` file onto the target area
...
- **Double-click to open** -- double-click any result to open the source file in Notepad
...
- **Errors and exceptions** -- full exception type, message, stack trace, and inner exceptions
...
When you click **Search**, the application first scans the directory recursively for `.evtx` files.
...
- **OS:** Windows (uses `wevtutil` for EVTX conversion, launches `notepad.exe` for file viewing)
